 ...
 *
 * @discussion This may be `nil` if the notification passed in `<[TwilioVoiceSDK handleNotification:delegate:delegateQueue:callMessageDelegate:]>`
...
/**
 * @brief Sends a user-defined message to the endpoints that subscribe to the user-defined messages of this Call Invite.
 *
 * @param callMessage The `TVOCallMessage` object with the message and configuration.
 *
 * @discussion The result will be raised to the `TVOCallMessageDelegate` provided in the
 ...
 *
 * @discussion A call message with content that does not match the content type will not result in the
 ...
 *
 * @discussion This method returns an empty string and will not send out the Call Message if the Call Invite has been accepted or rejected.
 *
...
NS_SWIFT_NAME(sendMessage(callMessage:));
 ...
 * callback when passed into `[TwilioVoiceSDK handleNotification:delegate:delegateQueue:callMessageDelegate:]`.
 *
